...
def get_activations(dataloader, num_images):
    """
    ...
    for count, data in enumerate(tqdm(dataloader)):
        images, _, _=data
        images=images.cuda()
        activations=inception_network(images)
        activations=activations.detach().cpu().numpy()
        batch_size=activations.shape[0]
        start_idx = batch_size * count
        end_idx = batch_size * (count + 1)
        inception_activations[start_idx:end_idx, :]=activations
    return inception_activations



def calculate_activation_statistics(dataloader, num_img):
    ...
    """
    act = get_activations(dataloader, num_img)
...
def calculate_fid(dataloader1, dataloader2, use_multiprocessing, num_img1, num_img2):
    ...
    print("calculating FID features of original dataset")
    mu1, sigma1 = calculate_activation_statistics(dataloader1, num_img1)
    print("calculating FID features of super-resolved dataset")
    mu2, sigma2 = calculate_activation_statistics(dataloader2, num_img2)
...
celeba="C:/Users/Asus/PycharmProjects/AutoEncoder/LOGS/Original_Images_Test"
lfw="C:/Users/Asus/PycharmProjects/datasets/aligned_lfw_all_in_one"
helen="C:/Users/Asus/PycharmProjects/datasets/Hellen_clean_aligned"
logs_path="C:/Users/Asus/PycharmProjects/AutoEncoder/LOGS/"
scale=16
algorithm="bicubic"
test_dataset_name="helen"
dest_path = logs_path+algorithm.upper()+str(scale)+"/sr_images_"+ test_dataset_name
if test_dataset_name=="celeba":
    src_path=celeba
elif test_dataset_name=="lfw":
    src_path=lfw
elif test_dataset_name=="helen":
    src_path=helen
...
if __name__ == "__main__":
    ...
    parser = OptionParser()
    parser.add_option("--p1", "--path1", dest="path1", default=src_path,
                      help="Path to directory containing the real images")
    parser.add_option("--p2", "--path2", dest="path2", default=dest_path,
    ...
    dataset1=datasets.ImageFolder(options.path1, transform=transform)
    dataloader1=DataLoader(dataset1, batch_size=options.batch_size, shuffle=False)
    num_image1=len(dataset1)
    dataset2 = datasets.ImageFolder(options.path2, transform=transform)
    dataloader2 = DataLoader(dataset2, batch_size=options.batch_size, shuffle=False)
    num_image2 = len(dataset2)
    fid_value = calculate_fid(dataloader1, dataloader2, options.use_multiprocessing, num_image1, num_image2)
    print(fid_value)